...
class GesturePredict(object):
    ...
    def __init__(self, configer, npz):
    ...
    def init_model(self):
        ...
        self.data_loader = DataLoader(
            Dataset(self.configer, self.data_path, self.npz, split="predict", data_type=self.data_type,
    ...
    def __predict(self):
        ...
        one_possibility = []
        with torch.no_grad():
            for i, data_tuple in enumerate(tqdm(self.data_loader, desc="Predict")):
                """
                input, gt
                """
                inputs = data_tuple[0].to(self.device)
                inputs2 = data_tuple[0].to(self.device)
                index = data_tuple[1].to(self.device) ## ground truth for predict is the frame number

                output, f = self.net(inputs, inputs2)
                possibility = torch.max(output.cpu().detach(), dim=1)
                one_possibility.append(possibility)
                predict = torch.argmax(output.detach(), dim=1)
                predict = predict.cpu().detach().numpy().reshape(-1)
                one_predict.append(predict)

                index = index.cpu().detach().numpy().reshape(-1)
                one_index.append(index)

        print('index: ', index)
        print('the predicted label is: ', one_predict)
        return one_index, one_predict,one_possibility

    def predict(self):
        index, f , p = self.__predict()
        return index, f, p
